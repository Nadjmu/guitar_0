...
def getTriadChromatic(root, index_list):
    ...
    for interval in index_list:
        note = chromatic_scale[(index_of_root + interval) % 12]
        notes.append(note)
    return notes
...
""", unsafe_allow_html=True)
        ...
        # Use st.header() instead of st.title() for smaller headings
    st.header("Chord Mood Tables")

    # Create columns for side-by-side tables
    col1, col2 = st.columns(2)

    with col1:
        st.subheader("Mood Summary")  # Even smaller heading
        markdown_table1 = """
        | Chord Type       | Emotional Vibe               | Genre Associations         |
        |------------------|------------------------------|----------------------------|
        | Maj7 / Maj9      | Dreamy, luxurious            | Jazz, Neo-Soul             |
        | Dom7 / 9 / 13    | Bluesy, resolving, funky     | Blues, Funk, Rock          |
        | m7 / m9 / m11    | Smooth, melancholic          | Neo-Soul, R&B              |
        | mMaj7 / dim7     | Dark, mysterious             | Jazz, Film Scores          |
        | 7#9 (Hendrix)    | Aggressive, psychedelic      | Rock, Fusion               |
        | Maj7♯11          | Bright, futuristic           | Fusion, Prog Rock          |
        | m7♭5 / ø         | Tense, unresolved            | Jazz, Metal (Diminished)   |
        """
        st.markdown(markdown_table1)

    with col2:
        st.subheader("Extended (Psych/Post-Rock Focus)")
        markdown_table2 = """
        | Chord Abbr. | Chord Type       | Emotional Vibe          | Genre Example Use                     |
        |-------------|------------------|-------------------------|---------------------------------------|
        | Maj7        | Major 7th        | Ethereal, expansive     | Slowdive, Sigur Rós                   |
        | Maj9        | Major 9th        | Dreamy, floating        | Tame Impala, Hammock                  |
        | Maj7♯11     | Lydian (#11)     | Bright, cosmic          | Pink Floyd, Radiohead                 |
        | m7          | Minor 7th        | Moody, introspective    | Explosions in the Sky                 |
        | m9          | Minor 9th        | Hypnotic, sultry        | This Will Destroy You                 |
        | m11         | Minor 11th       | Dark, swirling          | Godspeed You! Black Emperor           |
        | 7sus4       | Suspended 4th    | Suspenseful, open       | Mogwai, Mono                          |
        | 7#9         | Hendrix (#9)     | Psychedelic, raw        | Jimi Hendrix, King Gizzard            |
        | dim7        | Diminished 7th   | Dissonant, eerie        | Post-rock climaxes                    |
        | add9        | Added 9th        | Shimmery, uplifting     | U2, The Edge's arpeggios              |
        | m7♭5        | Half-diminished  | Haunting, unresolved    | Cinematic builds (Sigur Rós)          |
        """
    ...
    with type_col:
    # Basic chord types expander
        with st.expander("Basic Chords", expanded=False):
            basic_chords = ['Major', 'Minor', 'Diminished', 'Augmented']
            for chord in basic_chords:
                button_key = f"CTT{chord}"
                if button_key not in st.session_state.button_states_chords_tab:
                    st.session_state.button_states_chords_tab[button_key] = {'state': False}
                
                st.button(chord, key=button_key, 
                        on_click=btn_pressed_callback_chords_tab, 
                        args=(button_key,))
        
        # Seventh chords expander
        with st.expander("First Extensions (7th Chords)", expanded=False):
            seventh_chords = ['Major7', 'Dominant7', 'Minor7', 'Minor Major 7', 'Diminished7']
        ...
        # Ninth chords expander
        with st.expander("Second Extensions (9th Chords)", expanded=False):
            ninth_chords = ['Major9', 'Dominant9', 'Minor9', 'Add9', 'b9', '#9']
            for chord in ninth_chords:
                button_key = f"CTT{chord}"
                if button_key not in st.session_state.button_states_chords_tab:
                    st.session_state.button_states_chords_tab[button_key] = {'state': False}
                ...
                st.button(chord, key=button_key,
                        ...
                        args=(button_key,))
        # 11th chord expander
        with st.expander("Third Extensions (11th Chords)", expanded=False):
            eleventh_chords = ['Major11', 'Dominant11', 'Minor11', '#11']
            for chord in eleventh_chords:
                button_key = f"CTT{chord}"
                if button_key not in st.session_state.button_states_chords_tab:
                    st.session_state.button_states_chords_tab[button_key] = {'state': False}
                ...
                st.button(chord, key=button_key,
                        on_click=btn_pressed_callback_chords_tab,
                        args=(button_key,))
        # Suspended chords expander
        with st.expander("Suspended Chords", expanded=False):
            suspended_chords = ['sus4', 'sus2']
            for chord in suspended_chords:
                button_key = f"CTT{chord}"
                if button_key not in st.session_state.button_states_chords_tab:
                    st.session_state.button_states_chords_tab[button_key] = {'state': False}
    ...
    with info_col:
        ...
        if has_active_ctr_and_ctt():
            ...
            st.subheader(f"{root}{chord_type_abbr[chord_type.index(chord_type_1)]} ({' '.join(triad_chromatic)})")
